...
async function handleFaturamento(message, accountId, searchTerm, whatsappClients) {
  try {
    ...
    const { dados, dataInicio, dataFim } = await searchFaturamento(searchTerm);

    let response = buildFaturamentoMessage(dados, dataInicio, dataFim);
    ...
    if (dados.length > 0) {
      const imageBuffer = await gerarGraficoPizza(dados, 'Faturamento por Local', 'local');
...
function buildFaturamentoMessage(faturamento, dataInicio, dataFim) {
  const formatar = (data) => {
...
async function searchFaturamento(formattedSearch) {
