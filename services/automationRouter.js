...
const { handleEntregaProdutoViaFoto } = require('../handlers/handleEntregaProdutoViaFoto');
...
async function processAutomation(message, accountId, client) {
    ...
    // 🟢 Verifica comando de lote direto
    if (/^lote\s+\d+\s*\/\s*\d+[.,]?\d*$/i.test(text)) {
      const { handleGenerico } = require('../handlers/handleGenerico');
      await handleGenerico(message, accountId, text, { [accountId]: client }, {});
      return;
    }
    ...
    switch (handlerName) {
      ...
      case 'handleIncopisos':
        await handleIncoPisos(message, accountId, termoBuscado, { [accountId]: client });
        break;

      case 'handleEmbramaco':
        await handleEmbramaco(message, accountId, termoBuscado, { [accountId]: client });
        break;        
...
async function processAutomationImage(message, accountId, client) {
  ...
  switch (handlerName) {
    ...
    case 'handleEntregaProdutoViaFoto':
      await handleEntregaProdutoViaFoto(message, accountId, client);
      break;
